// src/lib/data.ts
import { Product, Portfolio, Metric, Roadmap, ReleaseGoal, ReleasePlan, ReleaseNote } from './types';
...
async function parseProductXML(filepath: string): Promise<Product | null> {
  try {
    ...
    // Parse metrics
    const metricElements = productEl.querySelectorAll('Metric, Metrics > MetricItem, Metrics > Metric');
...
export function getCurrentMonthData(product: Product) {
  ...
  const latestRoadmap = product.roadmap?.find(r => r.year === currentYear) || product.roadmap?.[0];
  const latestReleaseGoal = product.releaseGoals?.find(g => g.month === currentMonth && g.year === currentYear) || product.releaseGoals?.[0];
  const latestReleasePlan = product.releasePlans?.find(p => p.month === currentMonth && p.year === currentYear) || product.releasePlans?.[0];
  const latestReleaseNote = product.releaseNotes?.find(n => n.month === currentMonth && n.year === currentYear) || product.releaseNotes?.[0];
...
export function getPreviousMonthData(product: Product): { latestReleaseNote?: ReleaseNote } {
  ...
  const latestReleaseNote = product.releaseNotes?.find(n => 
