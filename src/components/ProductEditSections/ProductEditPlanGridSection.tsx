...
import { Save, Plus, Clipboard, Trash2, AlertTriangle } from 'lucide-react';
...
const ProductEditPlanGridSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    handlePlanMonthYearChange(month, year);
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const releasePlan = {
        ...
        month: planMonth,
        year: planYear,
        title: validItems.map(item => item.title).join('; ') || 'No plans defined',
        description: validItems.map(item => item.description).join('; ') || 'No description available',
        targetDate: new Date().toISOString(),
        status: 'planned' as const,
      ...
      if (success) {
        toast({
          title: "Success",
          description: "Release plan saved successfully",
        });
      } else {
        toast({
          title: "Error",
          description: "Failed to save release plan",
          variant: "destructive"
        });
      }
    } catch (error) {
