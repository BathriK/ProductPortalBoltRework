...
import { getCurrentMonthYear, getLatestVersionedItem } from '@/lib/utils';
      ...
      try {
        ...
        for (const portfolioData of portfolios) {
          ...
          if (foundProduct) {
            ...
            const filteredMetricsForPeriod = foundProduct.metrics?.filter(
              (m: Metric) => (m.month === metricsMonth && m.year === metricsYear)
            ) || [];

            const currentMetricParent = getLatestVersionedItem(filteredMetricsForPeriod);
